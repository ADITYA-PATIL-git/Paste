...
function POApproval({ navigation, route }) {
  ...
  const GetTransactionSearchPage = async (page, searchText = "") => {
    ...
    const Config = {
      Url: 'MobPurchaseOrder/GetTransactionSearchPage',
      method: 'POST',
      body: {
    ...
    try {
      ...
      const sortedResponse = safeResponse.sort(
        (a, b) => Number(b?.Ageing ?? 0) - Number(a?.Ageing ?? 0)
      );
  ...
  const groupByClaimNumber = (arr = []) => {
    ...
    const groups = arr.reduce((acc, item) => {
      const key = item?.ClaimNumber ?? 'UNKNOWN';
  ...
  const onClickDetailsApproval = (item, screenInfo) => {
    ...
    navigation.navigate('POApprovalDetail', { data: item, data2: screenInfo });
  };

  const workflow = item => {
    skipResetRef.current = true;
    navigation.navigate('POWorkflow', { data: item });
  };
                    ...
                    <View style={{ paddingHorizontal: 0 }}>
                      <View style={{ flexDirection: 'row', paddingTop: 10, alignItems: 'flex-start' }}>
                        <View style={commonStyles.InsNameSty}>
                          <Text style={commonStyles.gridtitle}>Initiator Name</Text>
                          <Text style={[commonStyles.gridtext, { paddingTop: 6 }]}>
                            {subItem?.InitiatorName}
                          </Text>
                        </View>

                        <View style={commonStyles.InsNameSty}>
                          <Text style={commonStyles.gridtitle}>Taxable Amount</Text>
                          <Text style={[commonStyles.gridtext, { paddingTop: 6 }]}>
                            {subItem?.ClaimAmount
