...
function VendorAdvanceApproval({ navigation, route }) {
  ...
  const GetTransactionSearchPage = async (page, searchText = "") => {
    ...
    const Config = {
      Url: 'MobVendorAdvance/GetTransactionSearchPage',
      method: 'POST',
      body: {
  ...
  const onClickDetailsApproval = (item, screenInfo) => {
    ...
    navigation.navigate('VendorAdvanceApprovalDetail', { data: item, data2: screenInfo });
  };

  const workflow = item => {
    skipResetRef.current = true;
    navigation.navigate('AdvanceWorkflow', { data: item });
  };
                    ...
                    <View style={{ paddingHorizontal: 0 }}>
                      <View style={{ flexDirection: 'row', paddingTop: 10, alignItems: 'flex-start' }}>
                        <View style={commonStyles.InsNameSty}>
                          <Text style={commonStyles.gridtitle}>Inward Date</Text>
...
export default VendorAdvanceApproval;
