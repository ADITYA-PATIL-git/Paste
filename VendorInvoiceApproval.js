...
function VendorInvoiceApproval({navigation, route}) {
  ...
  const handleRefresh = useCallback(() => {
    ...
    GetTransactionSearchPage(page); // Refreshes the first page
  }, [page]);
  ...
  const GetTransactionSearchPage = async (page, searchText = '') => {
    ...
    const Config = {
      Url: 'MobVendorInvoice/GetTransactionSearchPage',
      method: 'POST',
      body: {
    ...
    try {
      ...
      const pages =
        PAGE_SIZE > 0 && totalCount > 0 ? Math.ceil(totalCount / PAGE_SIZE) : 1;
      setTotalPages(pages);
      return;
    } catch (error) {
  ...
  const onClickDetailsApproval = (item, screenInfo) => {
    ...
    navigation.navigate('VendorInvoiceApprovalDetail', {
      data: item,
      data2: screenInfo,
    });
  };
  ...
  const workflow = item => {
    skipResetRef.current = true;
    navigation.navigate('VendorInvoiceWorkflow', {data: item});
  };
    ...
    return (
      <View
        ...
        <Text style={{color: '#000'}}>
          Page <Text style={{fontWeight: 'bold'}}>{currentPage}</Text> of{' '}
                  ...
                  <TouchableOpacity
                    ...
                    <View
                      ...
                      <View style={{flex: 1}}>
                        <Text
                          numberOfLines={2}
                          style={[commonStyles.CommHeaderName]}>
                          {subItem?.VendorName}
                        </Text>
                    ...
                    <View style={{paddingHorizontal: 0}}>
                      <View
                        ...
                        <View style={commonStyles.InsNameSty}>
                          <Text style={commonStyles.gridtitle}>
                            Inward Date
                          </Text>
                          <Text
                            style={[commonStyles.gridtext, {paddingTop: 6}]}>
                            {subItem?.ExpenseDate}
                          </Text>
                        </View>
                        ...
                        <View style={commonStyles.InsNameSty}>
                          ...
                          <Text
                            style={[commonStyles.gridtext, {paddingTop: 6}]}>
                            {subItem?.ClaimAmount
                              ? Number(subItem?.ClaimAmount).toLocaleString(
                                  'en-IN',
            ...
            ListEmptyComponent={() =>
              !initialLoader ? (
                <View style={{flex: 1, alignItems: 'center', marginTop: 40}}>
                  <Text
                    style={{fontSize: 16, color: '#7A7A7A', fontWeight: '500'}}>
                    No data available
                  </Text>
                </View>
              ) : null
